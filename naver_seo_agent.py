...
# ── 반복되기 쉬운 소재/접두 키워드 ──────────────────────────────────
REPEATED_PREFIX_TERMS = [
    "PVC", "PET", "PE", "PP", "ABS",
    "면", "니트", "린넨", "실크", "데님", "레이스", "가죽", "폴리",
]
...
11. 소재/접두 키워드 반복 금지
    — 예: PVC, 면, 니트 같은 단어를 복합 키워드 앞에서 여러 번 반복하지 말 것
    — 예: 'PVC버블풍선 PVC투명풍선' 대신 'PVC버블풍선 투명풍선'처럼 1회만 유지"""
...
def _remove_repeated_prefix_terms(text: str) -> str:
    """복합 토큰 앞에 반복되는 소재/접두 키워드를 1회만 남깁니다."""
    seen_prefixes: set[str] = set()
    normalized_tokens: list[str] = []

    for token in text.split():
        updated = token
        for prefix in REPEATED_PREFIX_TERMS:
            if not updated.startswith(prefix):
                continue
            remainder = updated[len(prefix):].strip()
            if prefix in seen_prefixes and remainder:
                updated = remainder
            else:
...
def clean_by_rules(name: str, original: str = "", top_keywords: list[str] | None = None) -> str:
    ...
    name = _remove_repeated_prefix_terms(name)
